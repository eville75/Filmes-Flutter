...
class _TimelineScreenState extends State<TimelineScreen> {
  ...
  Widget _buildItem(Pokemon p, int index) {
    return ListTile(
      leading: Image.network(p.imageUrl, width: 50, errorBuilder: (ctx, err, _) => Icon(Icons.error)),
      title: Text(p.name.toUpperCase()),
      subtitle: Text("Pokémon #${index + 1}"),
    );
  }
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index < _pokemons.length) {
            return _buildItem(_pokemons[index], index);
          } else {
            return Padding(
